...
class Favor(commands.Cog):
    ...
    async def favor_mod(self, ctx, char_name: str, faction: str, delta: int, *, notes: str = ""):
        guild_id = ctx.guild.id
        msg = await favor_service.mod_favor(guild_id, char_name, faction, delta, notes)
        await ctx.send(msg)
    ...
    async def favor_detail(self, ctx, char_name: str, faction: str):
        guild_id = ctx.guild.id
        f = await favor_service.get_detail(guild_id, faction, char_name=char_name)
        if not f:
    ...
    async def favor_remove(self, ctx, char_name: str, faction: str):
        guild_id = ctx.guild.id
        msg = await favor_service.remove_favor(guild_id, char_name, faction)
        await ctx.send(msg)
